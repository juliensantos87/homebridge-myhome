...
class OwnBlindAccessory extends OwnAccessory {
    ...
    setHoldPosition(hold, callback) {
        this.log("[" + this.id + "] Blind hold position  :" + hold);
    ...
    onData(packet) {
        ...
        if (extract) {
            ...
            if (direction == '0' && this.state != this.Characteristic.PositionState.STOPPED) {
                this.state = this.Characteristic.PositionState.STOPPED;
            } else if (direction == '1') {
...
class OwnThermostatAccessory extends OwnAccessory {
    ...
    updateCharacteristicCurrentTemperature(temperature) {
        this.log.error("[" + this.id + "] update CurrentTemperature (" + temperature + ")");
    ...
    updateCharacteristicTargetTemperature(temperature) {
        this.log.error("[" + this.id + "] update TargetTemperature (" + temperature + ")");
    ...
    updateCharacteristicTargetheatingCoolingState(state) {
        this.log.error("[" + this.id + "] update TargetheatingCoolingState (" + state + ")");
    ...
    updateCharacteristicCurrentHeatingCoolingState(state) {
        this.log.error("[" + this.id + "] update CurrentHeatingCoolingState (" + state + ")");
    ...
    onData(packet) {
        ...
        if (extract = packet.match(/^\*#4\*\d+\*0\*(\d+)##$/)) { // temperature not adjust by local offset
            ...
            this.log.info("[" + this.id + "] zone[" + this.zone + "] target temperature with adjust by local offset (" + this.temperature + ")");
        }
            ...
            this.log.info("[%s] zone[%s] local offset:%s (%s)", this.id, this.zone, this.localOffset, status);
        }       
            ...
            if (['1', '2'].includes(CV)) {
                this.log.info("[" + this.id + "] zone[" + this.zone + "] cooling ON");
                this.updateCharacteristicCurrentHeatingCoolingState(this.Characteristic.CurrentHeatingCoolingState.COOL);
            } else {
                this.log.info("[" + this.id + "] zone[" + this.zone + "] cooling OFF");
            }
            if (['1', '2'].includes(HV)) {
                this.log.info("[" + this.id + "] zone[" + this.zone + "] heating ON");
                ...
                this.log.info("[" + this.id + "] zone[" + this.zone + "] heating OFF");
            ...
            this.log.info("[" + this.id + "] zone[" + this.zone + "] actuator status (" + status + ")");
        ...
        //  Zone operation mode
            ...
            this.log.info("[" + this.id + "] zone[" + this.zone + "] operation mode (" + status + ")");
            ...
            if (value == 103) {
                ...
                this.log.info("[%s] zone[%s] operation mode Heating OFF", this.id , this.zone);
            }
            else if (value == 102) {
                this.updateCharacteristicTargetheatingCoolingState(this.Characteristic.TargetHeatingCoolingState.OFF);
                this.log.info("[%s] zone[%s] operation mode Anti Freeze", this.id , this.zone);
            }
                ...
                this.log.info("[%s] zone[%s] operation mode Heating program", this.id , this.zone);
            }
            else if (value > 13000 && value < 13255 ) {
                var days = value - 13000;
                this.updateCharacteristicTargetheatingCoolingState(this.Characteristic.TargetHeatingCoolingState.AUTO);
                this.log.info("[%s] zone[%s] operation mode Holiday program for %s day", this.id , this.zone, days);
            }
            else if (value == 21) {
                this.log.info("[%s] zone[%s] Remote control enabled", this.id , this.zone);
            }
            else {
            ...
            if (value == '110') {
                ...
                this.log.info("[%s] zone[%s] operation mode Manual Heating (%s)", this.id , this.zone,temp);
