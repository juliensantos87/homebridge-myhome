...
var myHomeLayer1 = function (params) {
	...
	this.pktToString = function (direction, packet) {
		var now = new Date();
		return now.toISOString()+' - '+MODE_STR[mode]+' id:'+this._id+' '+DIR_STR[direction]+' '+packet;
	};
	...
	this.parseMyOpenPacket = function (data) {
		if (self.reconnecthandle != null) { clearTimeout(self.reconnecthandle); self.reconnecthandle = null; }
	...
	this.connect = function () {
		...
		this.conn.on('error', (_error) => {
			...
			this.reconnecthandle = setTimeout(function() {
					console.log('Info[%s]: Try to reconnect !', MODE_STR[mode]);
					this.connect();
...
var myHomeLayer2 = function (params) {
	...
	this.startMonitor = function () {
		...
		this.monitor = new myHomeLayer1 (connparams);
